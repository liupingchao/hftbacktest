...
- `0528T002`: Compact replay lifecycle audit export for Stage 6 input is `待执行`.
...
14. `0528T002` has been created as the narrow implementation task for the `0527T001` QA recommendation and then narrowed to one route: implement a formal compact replay lifecycle audit export, with terminal lifecycle rows de-duplicated inside that compact export by Stage 6 semantics, and wire Stage 6 to consume the compact artifact by contract. It must not change strategy behavior, live behavior, fill/cancel replay semantics, queue/touch logic, parameters, guards, default-on behavior, or promotion state.
