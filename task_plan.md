...
- `0618T001`: Hyperliquid tiny-live minimal real-order executor and final gate repair has been created as the current next task. It must implement and QA a minimal live-capable Hyperliquid real-order executor, strictly preserving `10min / 0.01 BTC / post-only / max loss cap / cancel-all shutdown / artifact pullback`, then update and rerun the final go/no-go gate. It must not place real orders, create `0617T008`, run a 10-minute live test, disclose credentials, relax caps, or claim PnL/maker viability inside this task.
