...
- `0527T001`: Replay audit cancel_ack bloat planning/diagnosis for `0526T008` is `待验收`.
...
13. `0527T001` completed business-thread diagnosis and is `待验收`. It found the 21GB replay audit is primarily caused by audit replay lifecycle export / order-state tracking repetition: full replay audit has `32,893,719` `cancel_ack` rows but only `10,380` unique `order_id`s with cancel_ack, while the lifecycle-min input keeps exactly those first terminal facts for Stage 6. The next implementation boundary should be a narrow replay audit compact lifecycle export / terminal-order de-dup design, with Stage 6 consuming the compact lifecycle artifact by contract. This still does not authorize strategy changes, live, parameter search, or promotion.
