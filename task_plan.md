...
- `0622T004` is now `执行中`. It should convert the `0622T003` same-process watcher path from fixed-window batch candidate generation to event-driven rolling current-candidate evaluation, with target `candidate_event_to_guard_start <= 500ms` and hard fail-closed when selected current candidate age is `>1.0s`. It preserves `Alo`, dynamic size hard cap `<=0.005 BTC`, no taker/crossing, no one-tick-back, no cap relaxation, tracked cancel, independent open-orders proof, and T008 fail-closed.
