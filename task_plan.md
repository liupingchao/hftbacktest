...
- `0617T004` is `已通过`. It defined the Hyperliquid tiny-live signal / quote policy protocol and correctly failed closed on live threshold calibration. The next step is `0617T005` read-only signal / quote replay over accepted local cross-exchange artifacts. Do not create `0616T008` before replay/calibration QA passes.
...
- `0617T005`: Hyperliquid tiny-live read-only signal / quote replay has been created as a prepared follow-up only. It must not be treated as dispatched or executable until `0617T004` QA passes and total control explicitly dispatches it. Its intended scope is to replay the `0617T004` protocol over the existing accepted cross-exchange public/read-only datasets, producing trigger counts, side distribution, stale/data-gap rejection, quote crossing/post-only diagnostics, quote-distance distribution, and cap-trigger diagnostics. It remains no-live/no-order/no-private and cannot claim real fills, PnL, real inventory, post-only reject behavior, queue priority, deployment readiness, or maker viability.
...
- `0617T005` business execution completed and is awaiting QA. Full quote replay ran on the locally available `0601T005` pricing-signal rows, while the accepted seven-sample `0609T008` row-level artifact was used for basis-threshold distribution only because the original historical `pricing_signal_rows.csv` files are not present on this host. Final recommendation is `hyperliquid_tiny_live_signal_quote_replay_needs_threshold_calibration`; do not create `0616T008` yet.
