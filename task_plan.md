...
- `0622T004` has been created as `待执行` and is the prepared next M2 repair task. It should convert the `0622T003` same-process watcher path from fixed-window batch candidate generation to event-driven rolling current-candidate evaluation, with target `candidate_event_to_guard_start <= 500ms` and hard fail-closed when selected current candidate age is `>1.0s`. It preserves `Alo`, dynamic size hard cap `<=0.005 BTC`, no taker/crossing, no one-tick-back, no cap relaxation, tracked cancel, independent open-orders proof, and T008 fail-closed.
...
- `0622T004`: M2 event-driven current-candidate maker submit repair is `待执行` and prepared but not yet executed. It must repair the remaining batch-candidate half-life from `0622T003` by maintaining current in-memory L2/BBO and rolling trade-flow state, re-evaluating on each relevant public event, generating candidate context from the same current snapshot used by immediate guard, and failing closed when current touch / queue quality / age no longer satisfies the accepted gates. It may submit real orders only after tests, git-safe remote refresh, final gate go, current event-driven candidate eligibility, and immediate guard pass; it must still fail closed without live maker fill + complete T008 PnL evidence.
