...
- `0617T004` is `已通过`. It defined the Hyperliquid tiny-live signal / quote policy protocol and correctly failed closed on live threshold calibration. `0617T005` has now completed read-only signal / quote replay business execution and is awaiting QA. `0617T006` has been created as the next read-only optimistic PnL proxy task. Do not create `0616T008` before `0617T005` / `0617T006` QA/controller ratification.
...
- `0617T006`: Hyperliquid tiny-live read-only optimistic PnL proxy has been created as the next task. It may estimate a public-data theoretical upper bound under 100% theoretical intent fill assumptions, using future Hyperliquid mid labels and the `0617T004` / `0617T005` rule sources. It must reconcile the user's "6 datasets" wording against the actual accepted local manifests before computing results. It remains no-live/no-order/no-private and must not claim real fills, real PnL, fee/rebate settlement, spread capture, account inventory, execution quality, maker viability, deployment readiness, or live authorization.
- `0617T005`: Hyperliquid tiny-live read-only signal / quote replay has completed business execution and is awaiting QA. It replays the `0617T004` protocol over accepted local cross-exchange public/read-only datasets and remains no-live/no-order/no-private. It cannot claim real fills, PnL, real inventory, post-only reject behavior, queue priority, deployment readiness, or maker viability.
...
- No current prepared follow-up is authorized beyond `0617T006` until QA/controller review accepts or redirects the optimistic PnL proxy evidence.
...
- `0617T005` business execution was rechecked on this host and is awaiting QA. Full quote replay now consumes `8` locally available pricing-signal inputs: `0601T005` plus all `7` historical event-mode `pricing_signal_rows.csv` files referenced by `0609T008`. It replayed `161455` raw pricing rows and `26948` de-duplicated decision rows. The primary read-only calibration candidate is `75` ticks with `2` observations of persistence (`654` intents, `313` buy / `341` sell, `2.4269%` intent rate, `8/8` sample coverage), with `75` ticks and `3` observations as a stricter fallback (`327` intents, `1.2134%` intent rate). Final recommendation is `hyperliquid_tiny_live_signal_quote_replay_ready_for_qa`; do not create `0616T008` before QA/controller ratification.
- `0617T006` has been created and is waiting for business execution. It will compute only an optimistic read-only PnL proxy / theoretical upper bound, assuming theoretical maker intents fill at quote and using future public mid labels for settlement. It must report fixed-horizon and oracle-best-horizon estimates by sample/side/threshold/persistence/horizon and must not claim real PnL or execution viability.
