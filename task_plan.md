...
- `0615T001`: Basis-positive execution source-line real-readiness / read-only collector boundary design is the next formal task after `0612T001` QA passed. It must converge the local artifact skeleton / validator chain into a concrete next step toward real source-line implementation or read-only collector work by producing field-authority mapping, permission boundaries, runner consumption gate, next-task sequence, and convergence policy. It is design / boundary work only and does not authorize endpoint/source collector/runner implementation, private/order/account/live/economics data use, user stream, signing/nonce handling, real metrics, PnL proof, strategy/live/default-on/tiny-live readiness, deployment, promotion, or execution-layer maker viability proof. Controller convergence policy: after `0615T001` QA, at most `1` additional local-only task may be dispatched, and only for a concrete blocker named by `0615T001`; otherwise the next execution-proof task must move to real source-line implementation or read-only collector work.
