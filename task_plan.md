...
- `0622T003` is `待执行`. It is the next M2 repair task: merge public watcher, selected current candidate, immediate pre-submit guard, `Alo` submit/cancel, final open-orders proof, and T008 ledger into one `awsserver1` remote process so the watcher trigger no longer waits for controller artifact pullback and a separate live-window precheck. It preserves `Alo`, dynamic size hard cap `<=0.005 BTC`, no taker/crossing, no one-tick-back, no cap relaxation, tracked cancel, independent open-orders proof, and T008 fail-closed.
...
- `0622T003`: M2 same-process watcher-triggered maker order repair is `待执行`. It should remove the `0622T002` handoff latency by running watcher trigger, immediate current-candidate guard, private preflight, post-only `Alo` submit/cancel, final open-orders proof, pullback, and T008 ledger under one remote process. It may submit real orders only after tests, git-safe remote refresh, final gate go, public watcher eligibility, and immediate guard pass; it must still fail closed without live maker fill + complete T008 PnL evidence.
