...
- `0622T006` has been created as the next formal M2 repair task and is `待执行`. It targets the remaining `0622T005` exchange-side fast BBO drift / post-only validation blocker by adding a public-state anti-drift / touch-stability submit gate to the watcher-local inline reprice path. It must preserve `Alo`, unchanged dynamic size hard cap `<=0.005 BTC`, max `30` real submissions, no taker/crossing, no one-tick-back, no cap relaxation, public-only waiting, tracked cancel, independent open-orders proof, and T008 fail-closed.
...
- `0622T006`: M2 anti-drift maker-only submit gate / live calibration is `待执行`. Scope is one implementation plus controlled live-calibration task: extend the T005 inline watcher-local submit path with anti-drift / touch-stability gates using rolling public L2/trades state, record pass/block/reject matrices, and execute at most one bounded anti-drift-gated data-collection run after focused tests, remote refresh, and final gate go. It may place orders only when all maker-only gates pass; total submissions are capped at `30` with `Alo` and unchanged dynamic size hard cap `<=0.005 BTC`.
