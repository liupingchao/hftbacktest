...
- `0615T002`: Basis-positive private order response read-only collector boundary / implementation design is the next formal task after `0615T001` QA passed. It defines the first non-local source-line direction by specifying endpoint/permission boundaries, schema field handoff into `private_order_response_source.py`, redaction/local-storage policy, no-trading safety gates, future implementation QA gates, and next-task sequence. It does not authorize endpoint/source collector/runner implementation, credentials/signing/nonce/user stream, private/order/account/live/economics data use, order placement/cancellation, strategy/live/default-on/tiny-live readiness, real metrics, PnL proof, deployment, promotion, or execution-layer maker viability proof.
