...
- `0617T004` is `已通过`. It defined the Hyperliquid tiny-live signal / quote policy protocol and correctly failed closed on live threshold calibration. `0617T005` has now completed read-only signal / quote replay business execution and is awaiting QA. Do not create `0616T008` before `0617T005` QA/controller ratification.
...
- No current prepared follow-up is authorized by `0617T005` until QA/controller review accepts or redirects the replay evidence.
