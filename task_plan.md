...
- `0526T006`: Focused maker-edge design: inventory-aware quote placement is `待验收`.
...
8. `0526T006` has completed business-thread design output and is `待验收`. It defines one focused `inventory_aware_quote_placement_request` design: inventory state controls side preference and size pressure, fair/reservation edge is pricing context, quote-distance defines participation frontier, and stale/latency/post-only fields remain safety context only. It authorizes only a later read-only/default-off runner implementation task after QA, not strategy implementation, parameter search, live, default-on, or promotion.
