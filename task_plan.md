...
- `0622T002` is `待执行`. It is the next M2 task: implement/run a time-boxed public-only L2/trades watcher that waits for an existing `0622T001` fresh-touch / dynamic-size / session-gate eligible current-window candidate, then triggers at most one maker-only live micro-window with at most `2` post-only `Alo` submissions and `<=0.005 BTC` dynamic size cap. If no eligible window appears, no order is submitted and the task must emit no-eligible-window evidence. If a live trigger occurs but there is no fill or T008 cannot prove fee/inventory/realized PnL, M2 remains blocked.
...
- `0622T002`: M2 time-boxed public watcher with gated maker live trigger is `待执行`. Scope is intentionally one task: add/execute a bounded public-only watcher for L2/trades candidate windows, record depth/order-count/freshness/throughput/side/time evidence, and only if the existing fresh-touch quality gate allows a current candidate trigger one controlled post-only `Alo` live micro-window. It preserves the `0622T001` policy, dynamic size cap `<=0.005 BTC`, at most `2` real order submissions, tracked cancel, independent final open-orders proof, and T008 fail-closed. It must not switch to taker/crossing, use one-tick-back, relax caps, claim stable PnL, or enter M3.
