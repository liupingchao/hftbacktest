...
- `0617T006` business execution is complete and awaiting QA. The user selected `canonical_7` as the formal sample-set口径, so the prior exact-six ambiguity no longer blocks QA. At `75` ticks / persistence `2` / `1000ms`, official `canonical_7` reports `295.985 USDC` and `7/7` positive samples; diagnostic `0617T005_8_input` reports `299.38 USDC` and `8/8` positive samples. Final recommendation is `hyperliquid_tiny_live_optimistic_pnl_proxy_ready_for_qa`. This is not real PnL or execution viability.
