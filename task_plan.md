...
8. `0526T006` has been created but not dispatched. It should produce one focused maker-edge design around inventory-aware quote placement, using fair/reservation as pricing context and quote-distance plus size/side as execution-control dimensions. The task file has been tightened after `0526T004` QA to require a concrete candidate policy skeleton and a later read-only runner input/output contract.
