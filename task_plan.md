...
- `0623T003` business execution is complete and is now `待验收`. It splits anti-drift public flow taxonomy so buy-side sell-at-bid touch flow is classified as fill-support touch / visible queue depletion, while strict-through below limit remains adverse; sell-side handling is symmetric.
- `0623T003` changes anti-drift pressure blocking to use strict-through adverse quantity plus recent adverse BBO evidence. Touch-flow support alone no longer blocks, but it is not sufficient by itself and all other gates still apply. Focused verification passed with event-driven watcher/taxonomy tests `16 passed`, `py_compile`, watcher CLI help, and `git diff --check`. Local artifacts under `local_live_analysis/hyperliquid_tiny_live_m2_flow_taxonomy_0623T003/` cover touch-support pass, strict-through + adverse-BBO block, and mixed-flow pass cases. No live order window was run and no real order endpoint was called.
