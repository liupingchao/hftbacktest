...
from examples.hyperliquid import skhynix_c6in_latency as latency
from examples.hyperliquid import skhynix_c6in_latency_contracts as contracts
from examples.hyperliquid.test_skhynix_c6in_latency import (
    complete_population,
    schedule_rows,
    stringify_rows,
)
...
def test_gate2_blocks_before_credentials_when_minimum_notional_exceeds_cap(
    tmp_path: Path,
    ...
    assert market["minimum_order_notional_status"] == "blocked"
    assert market["quote_distance_safety_status"] == (
        "not_run_prior_authorization_block"
    )
