...
def compute_segments(total_duration_seconds: float, segment_duration_seconds: float) -> list[float]:
...
def _auxiliary_reconnect_intervals(
    *,
    track_id: str,
    ...
    if not isinstance(disconnect_events, list) or len(disconnect_events) != reconnect_count:
        failures.append(f"hyperliquid_auxiliary_reconnect_count_mismatch:{track_id}")
        return [], failures
    if connection_attempt_count != reconnect_count + 1:
        failures.append(f"hyperliquid_auxiliary_connection_attempt_mismatch:{track_id}")
    if not expected_identities:
        failures.append(f"hyperliquid_auxiliary_expected_subscriptions_missing:{track_id}")
    if track_manifest.get("all_required_subscription_acks_received") is not True:
        failures.append(f"hyperliquid_auxiliary_subscription_ack_incomplete:{track_id}")
    if track_manifest.get("raw_row_count_reconciled") is not True:
        failures.append(f"hyperliquid_auxiliary_raw_row_count_mismatch:{track_id}")
    if int(track_manifest.get("parse_error_count", -1)) != 0:
        failures.append(f"hyperliquid_auxiliary_parse_error:{track_id}")
    if failures:
        return [], failures
    ...
    for item in disconnect_events:
        if not isinstance(item, dict) or int(item.get("disconnect_local_ts", 0)) <= 0:
            failures.append(f"hyperliquid_auxiliary_disconnect_invalid:{track_id}")
            continue
    ...
    for index, (disconnect_ts, disconnect) in enumerate(disconnects):
        ...
        for identity, timestamps in ack_timestamps.items():
            ...
            if recovered <= 0:
                failures.append(
                    f"hyperliquid_auxiliary_reconnect_ack_missing:{track_id}:"
        ...
        for channel, timestamps in channel_timestamps.items():
            ...
            if previous <= 0:
                failures.append(
                    f"hyperliquid_auxiliary_pre_disconnect_data_missing:{track_id}:{channel}"
                )
            if resumed <= 0:
                failures.append(
                    f"hyperliquid_auxiliary_resumed_data_missing:{track_id}:{channel}"
                )
        ...
        if duration_seconds > max_interval_seconds:
            failures.append(f"hyperliquid_auxiliary_degraded_interval_above_gate:{track_id}")
            continue
        intervals.append(
            {
                "track_id": track_id,
    ...
    if len(intervals) != reconnect_count and not failures:
        failures.append(f"hyperliquid_auxiliary_degraded_interval_count_mismatch:{track_id}")
...
def validate_profile_sample(
    *,
    ...
    if int(binance.get("reconnect_count", -1)) != 0:
    ...
    for track_id in sorted(required_tracks.intersection(tracks)):
        ...
        if track.get("quality", {}).get("passes") is not True:
            failures.append(f"hyperliquid_track_quality_failed:{track_id}")
        raw_path = (
        ...
        if reconnect_count != manifest_reconnect_count:
            failures.append(f"hyperliquid_track_reconnect_manifest_mismatch:{track_id}")
        elif track_id in HYPERLIQUID_CORE_TRACKS and reconnect_count != 0:
            failures.append(f"hyperliquid_core_track_reconnect_nonzero:{track_id}")
        elif track_id in HYPERLIQUID_AUXILIARY_TRACKS:
            if reconnect_count < 0:
                failures.append(f"hyperliquid_auxiliary_reconnect_count_missing:{track_id}")
            elif reconnect_count > 0 and raw_path.is_file():
                intervals, reconnect_failures = _auxiliary_reconnect_intervals(
                    track_id=track_id,
                ...
                degraded_intervals.extend(intervals)
...
class CollectionCampaignSupervisor:
    ...
    def _payload(self, *, state: str, phase: str, **extra: Any) -> dict[str, Any]:
        return {
            ...
            "execution_mode": (
                "postprocess_only" if self.args.postprocess_only else "collect_and_postprocess"
            ),
    ...
    def load_existing_collections(
        ...
        *,
        ...
        for index, duration_seconds in enumerate(durations, start=1):
            ...
            if child_payload.get("passes") is not True:
            ...
            for profile_id in profiles:
                child = child_results[profile_id]
                if not isinstance(child, dict) or int(child.get("returncode", -1)) != 0:
                    raise SupervisorError(
                ...
                collection_task_id = str(sample_manifest.get("task_id", ""))
