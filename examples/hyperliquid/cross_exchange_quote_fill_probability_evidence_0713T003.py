#!/usr/bin/env python3
"""Offline quote/fill evidence rerun using the accepted 0713T002 resting-interval package."""
...
def opportunity_status(trade_status: str, order_status_type: str) -> str:
    if trade_status == "public_flow_artifact_missing":
...
def build_attempt_rows(
    *,
    ...
    public_trade_rows = read_csv_rows_if_exists(window_dir / "resting_interval_public_trades.csv")
    ...
    for evaluation_id, quote in enumerate(quote_rows, start=1):
        ...
        if order_called and order_status == "resting":
            ...
            evidence_limitation = "no_attempt_keyed_interval_public_trades;proxy_only_lifecycle_depth" if public_trade_count == 0 else "resting_interval_public_trades_present"
            route_signal = "public_flow_artifact_gap" if trade_status in {"public_flow_artifact_missing", "rolling_proxy_present_resting_interval_missing"} else "censored_resting_no_fill"
            public_trade_summary_rows.append(
                {
                    ...
                    "capture_status": "captured_for_resting_attempt" if public_trade_count else "no_matching_attempt_keyed_public_trades",
                }
            )
            attempt_rows.append(
                {
    ...
    return attempt_rows, public_trade_summary_rows, censoring_rows, depth_proxy_rows
...
def choose_recommendation(attempt_rows: list[dict[str, Any]]) -> str:
    ...
    if any(row.get("trade_through_status") in {"public_flow_artifact_missing", "rolling_proxy_present_resting_interval_missing"} for row in attempt_rows):
...
def run_analysis(*, source_root: Path, output_dir: Path) -> dict[str, Any]:
    ...
    validation_summary = read_json(source_root / "0713T002_local_validation_summary.json")
    source_overlay = read_json(source_root / "source_attribution_overlay.json")
    ...
    for window_dir in window_dirs:
        ...
        window_attempts, window_public_trade_summary, window_censoring, window_depth_proxy = build_attempt_rows(
    ...
    shutil.copy2(source_root / "source_attribution_overlay.json", output_dir / "source_attribution_overlay.json")
    ...
    input_manifest = {
        ...
        "source_validation_summary": {
            "remote_raw_file_count": validation_summary.get("remote_raw_file_count", 0),
            "local_raw_file_count_before_validation": validation_summary.get("local_raw_file_count_before_validation", 0),
            "sha256_reconciliation_status": validation_summary.get("sha256_reconciliation_status", ""),
            "json_parse_error_count": validation_summary.get("json_parse_error_count", 0),
    ...
    manifest = {
        ...
        "route_rationale": (
            "The accepted live window still lacks attempt-keyed interval public trades and exact interval reconstruction; "
            "the resting attempt remains proxy-censored and cannot support quote/fill probability or quote-policy claims yet."
        ),
        "supported_no_fill_reasons": [
            "no_attempt_keyed_interval_public_trades",
            "insufficient_exact_interval_public_flow_reconstruction",
            "short_horizon_censoring",
        ],
        "unsupported_claims": [
            "synthetic_fill",
            "fee",
