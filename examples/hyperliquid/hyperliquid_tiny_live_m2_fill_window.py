...
def valid_reference_oid(value: Any) -> bool:
    if isinstance(value, bool):
        return False
    if isinstance(value, int):
        return 0 <= value <= executor.MAX_REFERENCE_OID
    return (
        isinstance(value, str)
        and bool(value)
        and len(value) <= len(executor.MAX_REFERENCE_OID_TEXT)
        and value.isascii()
        and value.isdecimal()
        and (value == "0" or value[0] != "0")
        and (
            len(value) < len(executor.MAX_REFERENCE_OID_TEXT)
            or value <= executor.MAX_REFERENCE_OID_TEXT
        )
    )
...
def historical_reference_tokens(
    row: dict[str, Any],
    *,
    reason_prefix: str,
) -> tuple[dict[str, str], list[str]]:
    ...
    for kind, aliases in aliases_by_kind.items():
        ...
        for alias in aliases:
            ...
            if not valid_raw:
                ...
                continue
            candidates.append(reference_identity_token(kind, raw))
            ...
            if (
                ...
                for alias, token in alias_token_map.items():
                    if (
                        ...
                        or not valid_reference_identity_token(
                            kind,
                            token,
                        )
                    ):
...
def historical_reference_row_classification(
    row: Any,
    *,
...
def _cancel_reference_reconciliation_v2(
    *,
...
def terminal_query_status_from_result(
    ...
    *,
    ...
    if method == "historical_orders":
        ...
        matches: list[dict[str, Any]] = []
        for row in rows:
            classification = historical_reference_row_classification(
                row,
...
def terminal_query_row_has_historical_semantics(row: Any) -> bool:
    if not isinstance(row, dict):
        return False
...
def terminal_query_attempt_audit(
    *,
    ...
    raw_historical_semantics_present = any(
        terminal_query_row_has_historical_semantics(row)
    ...
    if any(
        terminal_query_method_result_mismatch(row)
        for row in raw_query_rows
    ):
        reasons.append(
    ...
    *,
    ...
    if (
        ...
        for raw_final_order in final_open_orders:
            ...
            final_tokens, final_reasons = normalized_reference_tokens(
                final_order,
    ...
    for query_index, raw_query in enumerate(terminal_query_results):
        ...
        attempt = strict_positive_attempt(query.get("attempt"))
        identity_tokens, reasons = normalized_reference_tokens(
            query,
