...
from examples.hyperliquid import (
    skhynix_fixed_epoch_leader_trigger_opposition_veto_a_minus1_formal_replacement as AUDIT,
)
from examples.hyperliquid import (
    skhynix_fixed_epoch_leader_trigger_opposition_veto_a_minus1_formal_replacement_verifier as VERIFIER,
)
...
def test_formal_baseline_authority_call_requires_clean_worktree(
    ...
    calls = [
        ...
        and node.func.value.id == "baseline_verifier"
        and node.func.attr == "verify_authority"
    ]
    assert len(calls) == 1
    assert [
        keyword.arg
        for keyword in calls[0].keywords
        if keyword.arg == "check_working_tree"
    ] == ["check_working_tree"]
    keyword = next(
        item for item in calls[0].keywords if item.arg == "check_working_tree"
    )
    ...
    monkeypatch.setattr(
        AUDIT.baseline_verifier,
        "load_manifest",
        lambda actual_repo: manifest if actual_repo == repo else None,
    )
    ...
    def verify(
        ...
        *,
        ...
        observed.append((actual_repo, actual_manifest, check_working_tree))
        return expected

    monkeypatch.setattr(AUDIT.baseline_verifier, "verify_authority", verify)
    assert AUDIT.verify_baseline_authority(repo) is expected
    assert observed == [(repo, manifest, True)]
...
def test_detector_child_rejects_inherited_regular_file_fd(tmp_path: Path) -> None:
    ...
    code = (
        "from examples.hyperliquid import "
        "skhynix_fixed_epoch_leader_trigger_opposition_veto_a_minus1_"
        "formal_replacement as a;"
        "import sys;"
...
def synthetic_terminal_package(
    ...
    *,
    ...
    write_ascii(repo / VERIFIER.TASK_PATH, "task: 0830T003 synthetic fixture\n")
    formal_argv = [
    ...
    result_out = repo / ".workflow/reports/0830T003-terminal-verifier.json"
