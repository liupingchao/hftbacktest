...
from account_inventory_read_only_source import (
...
def test_transform_rows_redacts_and_validates() -> None:
    ...
    assert "synthetic_account_scope_alpha" not in artifact_text
    assert "synthetic_future_order_alpha" not in artifact_text
    assert result.rows[0]["account_scope_id"].startswith("account_scope_sha256_")
    assert result.rows[0]["related_future_opaque_order_ref_hash"].startswith("future_order_sha256_")
...
def test_generate_artifacts_writes_expected_outputs(tmp_path: Path) -> None:
    ...
    output_artifact = tmp_path / "account_inventory_output_artifact.csv"
    summary = tmp_path / "account_inventory_validation_summary.csv"
    ...
    assert {row["raw_future_order_ref_persisted_in_artifact"] for row in redaction_rows} == {"False"}
