# SKHYNIX c6in Hyperliquid Execution Latency Measurement Plan V2

Date: 2026-08-22

Revision: execution contract 2

Status: the user approved the exact monetary-cap revision and explicitly
authorized the subsequent private reads, post-only orders, cancels and
reduce-only flatten safety path on 2026-08-22. Execution contract 2 supersedes
contract 1 for future execution only. Formal task `0822T001` and its Gate 2
blocker evidence remain immutable. Contract 2 may be executed only through
...
This revision is outcome-blind and changes only monetary risk limits:

```text
per_order_notional_cap_usdc: 5 -> 15
aggregate_position_cap_usdc: 10 -> 30
max_loss_usdc: 1 -> 3
```

Attempt counts, batch counts and durations, sample gates, quote distance,
settle time, inter-attempt spacing, latency statistics, reliability gates and
all other non-monetary controls remain unchanged. The Hyperliquid venue
minimum is an external market rule and is not multiplied by this revision.
...
currently available measurement route, and the user explicitly authorized
the contract 2 live envelope on 2026-08-22.
...
available route and uses the explicitly authorized live envelope under §8.
...
The `15 / 30 / 3 USDC` values are exactly three times the contract 1 monetary
limits. They do not authorize larger-than-minimum measurement orders. Gate 2
must recompute the current lot-rounded executable minimum and fail closed if
it exceeds either applicable notional cap. The contract 1 observation of
`0.009 SKHX = 11.2149 USDC` explains this revision but is not a substitute for
the fresh contract 2 preflight.
...
an ex-ante guarantee that realized loss cannot exceed 3 USDC. The temporary
...
the realized flatten slippage is at or above 3 USDC, the entire task stops and
...
16. `max_loss_usdc=3` means realized adverse price slippage from the
...
20. Contract 2 triples only the monetary envelope from `5 / 10 / 1 USDC` to
    `15 / 30 / 3 USDC`. It does not scale attempts, durations, quote distance,
    sample gates or any other non-monetary control. The user's 2026-08-22
