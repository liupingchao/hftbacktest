...
## 22. Historical A0-A4 Execution Result, 2026-08-27

Task `0827T004` executed the outcome-blind A0-A4 path on the existing Binance
SKHYNIXUSDT public data.

### 22.1 Frozen historical roles

The execution admitted:

- 29 formal captures across 9 research dates;
- 35.917 hours of public market data;
- 1,292,945 valid 100ms reconstruction rows;
- zero declared or observed `U/u/pu` continuity gaps;
- July 29 for prior-session normalization calibration;
- July 30, August 3 and August 4 for method development;
- August 7, August 24 and August 25 for blocked validation;
- August 26 and August 27 for chronological no-refit historical replay.

No session is called prospective. All admitted data existed before this
execution protocol was frozen.

The reconstruction grid is 100ms. The state observation grid is 200ms because
the maximum capture-level depth interarrival p99 was 155.858ms. Neither grid is
interpreted as a phase or cycle duration.

### 22.2 Primary model result

The selected neutral discrete model was the six-state diagonal multivariate
Student-t emission, shifted-negative-binomial duration sticky HSMM.

On blocked validation:

| Model | Mean log predictive density |
| --- | ---: |
| selected Student-t HSMM, K=6 | -149.570 |
| Student-t memoryless HMM, K=6 | -152.841 |
| Gaussian HSMM, K=6 | -2045.884 |
| single-state Student-t null | -184.724 |
| continuous ridge VAR(1) | **-127.672** |

The continuous VAR(1) baseline beat the selected discrete model by about
21.898 log-density units per model row. The matched state-profile correlation
was 0.900 on average, but its minimum was only 0.468. Stable-looking state
profiles therefore did not establish that discrete phases were the best
description of held-out structure.
...
This is the first formal failure at the state-stability gate. It blocks a
positive interpretation of all downstream discrete grammars.

### 22.3 Time-scale sensitivity

The selected HSMM was rescored with explicit duration support widened from:

```text
12.8s -> 25.6s -> 51.2s
```

The corresponding validation scores were:
...
The failure is therefore not explained by selecting a duration support that
was too short.

The 400ms robustness replay had label-matched agreement of:

- 0.9958 on blocked-validation captures;
- 0.9961 on no-refit historical replay captures.
...
per second. Most state dwell medians were exactly one 200ms model step. The
most common paths were short alternating returns such as:
...
Twenty-one longer, at-least-three-state cycle paths met diagnostic
cross-session count rules, and the leading transition-permutation null
p-value was 0.02439. These are not accepted phase grammars because the
upstream discrete-state gate failed. Recurrent paths inside an inferior
discretization do not prove that the market is generated by discrete phases.

The causal online filter also failed:

- no-refit replay mean run recall: 0.365;
- detections occurring only at the effective end of a run: 0.529;
- true prospective session count: 0.

### 22.5 Decision

The original alignment hypothesis is not rejected in its broadest form:
repeatable spatial-temporal market structure may still exist. What this
execution rejects is the stronger current implementation:

```text
interpretable features
  -> small discrete Student-t HSMM state alphabet
  -> recurrent maximal-run grammar
  -> causally recognizable phase cycle
```

For these data, structure is better modeled as a continuously evolving
autoregressive flow state. Track B and N/S/P/R semantic mapping remain closed.
Any future continuation should first investigate continuous latent trajectory
alignment, change-point-localized excursions, or recurrence in continuous
state-space paths. It should not tune another event horizon or force a
four-phase ontology onto the existing decoder.

The auditable local artifact root is:
