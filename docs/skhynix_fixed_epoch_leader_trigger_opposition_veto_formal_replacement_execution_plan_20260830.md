# SKHYNIX Fixed Epoch Leader Trigger With Opposition Veto A-1 Formal Replacement Execution Plan
...
Revision: 1, pre-execution formal replacement

## 0. Replacement Boundary

This is a new formal task and a new one-shot attempt. It does not reopen,
repair or replace the immutable execution history of `0830T002`.

The scientific hypothesis, primary/sensitivity variants, thresholds, gates,
prediction, source-cache authority and all outcome-blind restrictions are
inherited unchanged from the independently accepted Revision 18 plan:
...
`0830T002` produced no Build A/B/P package, no observed support counts and no
scientific classification. Therefore this replacement contains no
data-dependent threshold, feature, gate, prediction or analysis change.

The only registered behavior change is the baseline authority preflight call:

```python
baseline_verifier.verify_authority(
    repo_root,
    baseline_manifest,
...
The replacement has distinct runner, verifier, tests, task/claim/receipt
paths, attempt root, implementation/consumption/terminal tags and remote
controller ref. Formal execution remains locked until this replacement plan
and implementation independently pass review.
...
python \
  ...
  --formal-attempt \
  --repo-root /Users/liu/Documents/hftbacktest-0830t003-leader-trigger-formal-replacement \
  --source-cache-root \
...
```text
commit message:
  audit: consume 0830T003 formal attempt claim
annotated tag:
...
commit message:
  audit: seal 0830T003 formal attempt result
annotated tag:
skhynix-fixed-epoch-leader-trigger-a-minus1-replacement-terminal-v1
```
...
consumption push:
  git push --porcelain origin \
    <consumption_head>:refs/heads/codex/0830T003-controller-ledger
...
terminal push:
  git push --porcelain origin \
    <terminal_head>:refs/heads/codex/0830T003-controller-ledger
...
```text
.workflow/attempt-claims/0830T003.armed.json
```
...
skhynix_fixed_epoch_leader_trigger_opposition_veto_a_minus1_formal_replacement_verifier.py
```
...
python \
  ...
  --verify-terminal \
  --repo-root /Users/liu/Documents/hftbacktest-0830t003-leader-trigger-formal-replacement \
  --attempt-root /Users/liu/Documents/hftbacktest-0830t003-leader-trigger-formal-replacement/local_live_analysis/skhynix_fixed_epoch_leader_trigger_opposition_veto_a_minus1_formal_replacement_0830T003_formal_v1 \
  --implementation-tag skhynix-fixed-epoch-leader-trigger-a-minus1-replacement-implementation-v1 \
  --consumption-tag skhynix-fixed-epoch-leader-trigger-a-minus1-replacement-consumed-v1 \
  --terminal-tag skhynix-fixed-epoch-leader-trigger-a-minus1-replacement-terminal-v1 \
  --result-out /Users/liu/Documents/hftbacktest-0830t003-leader-trigger-formal-replacement/.workflow/reports/0830T003-terminal-verifier.json
```
...
```text
.workflow/attempt-claims/0830T003.claimed.json
```
...
```text
.workflow/attempt-claims/0830T003.{armed,claimed}.json = object{
...
```text
.workflow/attempt-receipts/0830T003.terminal.json = object{
  schema_version:int,task_id:str,attempt_id:str,status:str,
...
```text
.workflow/reports/0830T003-terminal-verifier.json = object{
  schema_version:int,task_id:str,attempt_id:str,status:str,
...
tracked_files count/order:
  ...
  2 docs/skhynix_fixed_epoch_leader_trigger_opposition_veto_formal_replacement_execution_plan_20260830.md
  3 .workflow/tasks/0830T003.md
  4 examples/hyperliquid/skhynix_fixed_epoch_leader_trigger_opposition_veto_a_minus1_formal_replacement.py
  5 examples/hyperliquid/skhynix_fixed_epoch_leader_trigger_opposition_veto_a_minus1_formal_replacement_verifier.py
  6 examples/hyperliquid/test_skhynix_fixed_epoch_leader_trigger_opposition_veto_a_minus1_formal_replacement.py
  7 .workflow/attempt-claims/0830T003.claimed.json
