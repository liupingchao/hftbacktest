...
`P0/P1/P2/P3=0/2/1/0`. It changes only execution governance,
failure recovery and review provenance. It does not change or rerun:
...
Package admission must load the task, matrix, V3 plan, V3 review and all four
runtime/test files from that Git commit. The packaged copies must equal those
Git object bytes. The accepted-input task SHA must equal both the packaged
task SHA and the pinned execution-authority task SHA.
...
The attempt root must not exist before start. A durable fsynced
...
Every phase update is canonical, fsynced and replaces only the mutable attempt
receipt. Evidence files are never deleted or overwritten.
...
An uncatchable process termination may leave `status=running`. Recovery may
mark it `interrupted` only when the recorded PID is no longer alive. Recovery
must inventory every existing evidence entry before updating the receipt.
...
The validator reloads every candidate object from Git and recomputes all
identities. `review_absent_in_candidate`,
`review_submission_absent_in_candidate`,
...
candidate_tree_sha256=<exact candidate tree>
...
The reviewer actor ID must differ from the controller actor ID. The review
must be committed after the candidate receipt, and neither review nor
candidate receipt may exist in the candidate commit.

This proves repository-visible revision separation, exact candidate binding
and distinct workflow actor identifiers. It is not a cryptographic proof of
human identity. Stronger personhood or organizational-signature claims would
require an external signing service and are outside this task.
...
5. packaged task/runtime/matrix bytes differing from commit `71adbfa6` fail;
6. a candidate commit containing its own review or receipt fails;
7. same controller/reviewer actor IDs fail;
8. candidate commit/tree/runtime drift fails;
9. attempt receipt exists before any build root is created;
10. a caught failure writes a durable failed receipt and preserves partial
    evidence;
11. a dead-PID running attempt can be marked interrupted;
12. a live-PID running attempt cannot be marked interrupted;
13. duplicate attempt IDs fail without changing the prior attempt;
14. an occupied package staging path fails without deletion;
15. the current and frozen hostile suites reject every declared mutation with
    `fail_open_count=0`.
