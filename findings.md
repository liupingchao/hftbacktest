...
- `0527T001` completed planning/diagnosis-only execution. The `0526T008` full replay audit has `32,893,719` `cancel_ack` rows but only `10,380` unique cancel-ack `order_id`s; top repeated orders each appear over `325k` times. The source is primarily audit replay lifecycle export / order-state tracking repetition: `OrderLifecycleTracker.observe()` retains current snapshots from `hbt.orders(0)`, and audit replay overlay writes both regular cancel_ack rows and forced live terminal constraint rows as decisions advance. Repeated rows are semantically redundant for Stage 6 once the first terminal fact per order is kept. The recommended next implementation boundary is a narrow compact lifecycle replay audit artifact or writer-side terminal-order de-dup, not strategy change or Stage 6 methodology change.
