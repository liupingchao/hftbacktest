...
QA验收方式：
- V4 Round 6 已完成 exact candidate receipt 和独立接受；controller
  transition receipt 生效并完成最终状态 Gate 0 重放后派发 QA Round 2。
...
- business report ends at `待验收`; QA Round 2 is the next independent
  acceptance boundary. No controller acceptance or final signal claim is
  made before QA accepts the immutable authority, transition receipt and
  final-state hostile/Gate 0 evidence.
