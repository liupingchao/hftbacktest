...
- `run_order_once` 接入 runtime envelope，且 max-loss 先于普通 envelope rejection 执行 kill-switch。
...
- 新增 focused coverage：最坏双向暴露、cancel-pending/inflight、聚合多层订单、reduce-side、严格 caps、submission budget、提交前阻断及 max-loss 优先级。
