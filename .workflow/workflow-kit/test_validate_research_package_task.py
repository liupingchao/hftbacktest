...
def test_new_unclassified_task_fails_closed(tmp_path):
    task = tmp_path / "new.md"
    task.write_text(
        """# task
...
""",
    ...
    with pytest.raises(TrustKernelError) as caught:
        validator.validate_task(task, None)
    assert caught.value.code == "TASK_CLASSIFICATION_REQUIRED"
