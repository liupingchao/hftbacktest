...
If that QA passes, the next planned task should be a narrow Step 5C default-off / diagnostic-first implementation contract only for anchor arbitration, side-conservative rounding, clamp, post-clamp re-check, guarded fallback, stale/join-age suppression, and diagnostic counters. It should not widen into source-level drift repair, generic quote-control redesign, or live promotion.
```
...
Current task: `0518T003` is waiting for QA as the dependent Step 5B read-only diagnostic after `0518T002` QA passed.
