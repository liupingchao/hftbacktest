...
- `0601T005` has completed business execution as the Binance-led Hyperliquid read-only pricing-signal runner implementation and is waiting for QA.
...
- Prepared read-only pricing-signal runner: `0601T005` (`待验收`, unblocked by `0601T004` QA)
...
- `0601T005` completed business execution. The read-only runner generated `21541` pricing signal rows, `4` feature quality rows, `30` horizon label summary rows, `540` feature/regime rows, and `54` venue-state conditioning rows from `3596` primary rows. Recommendation is `keep_for_read_only_research`; this remains public-artifact read-only research only and does not authorize strategy/private/order/live/parameter/default-on/tiny-live/promotion.
